...
public class ChannelService {
    ...
    public Channel show(int id){
        Channel channel = dao.findById(id).orElse(null); // 정보 가져오기
        Member member = memberDAO.findById(channel.getMember().getId()).orElse(null); // channel.getMember().getId()) = member의 ID
